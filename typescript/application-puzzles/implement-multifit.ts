...
const multifit = (numberArray: number[], binMax: number, binIncrease=false): number[][] => {
  ...
  let sweepCheck = false;

  numberArray.forEach((currentNumber: number) => {
    console.log(`binRow: ${binRow}. currentNumber: ${currentNumber}. binMax: ${binMax}`);
